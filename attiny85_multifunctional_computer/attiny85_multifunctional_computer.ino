...
int read_button(){
  ...
  delay(1);
  garbage=analogRead(inputpin);
  delay(20);
  val0=analogRead(inputpin);
  delay(1);
  val1=analogRead(inputpin);
  delay(1);
  val2=analogRead(inputpin);
  delay(1);
  val3=analogRead(inputpin);
  delay(1);
  val4=analogRead(inputpin);
  delay(1);
  val5=analogRead(inputpin);
  delay(1);
  val6=analogRead(inputpin);
  delay(1);
  val7=analogRead(inputpin);
...
void memory_game() {
 randomSeed(millis());
 int byteA,byteB;
 byteA=random(1,127);//random byte
 print_byte(byteA);
 clearLEDs();
 delay(2000);
 prompt_for_input(); 
...
void guess_my_number() {
 randomSeed(millis());
 int byteA,byteB;
 byteA=random(1,15);//random byte
  while(byteA!=byteB) {
     prompt_for_input(); 
     byteB=get_byte(); 
     if(byteB>byteA) {//too High
       flashLEDs();
       flashLEDs();
       flashLEDs();
       flashLEDs();
...
void loop() {
  ...
  edit_nibble();
  if(nibble.bit0) ;
  if(nibble.bit1) memory_game();
  if(nibble.bit2) guess_my_number();
  if(nibble.bit3) calc();
